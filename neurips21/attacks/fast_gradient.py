...
def fgm(model, x, norm=np.inf, eps=0.01, epochs=1, clip_min=0.0, clip_max=1.0):
    """
    ...
    def _body(xadv, i):
        ...
        xadv = tf.stop_gradient(xadv + eps * grad)
...
def fgmt(model, x, norm=np.inf, y=None, eps=0.01, epochs=1, clip_min=0.0, clip_max=1.0):
    """
    ...
    if y is None:
        ...
        indices = tf.cond(
            tf.equal(0, tf.rank(y)), lambda: tf.zeros([n], dtype=tf.int32) + y, lambda: tf.zeros([n], dtype=tf.int32)
        )

    target = tf.cond(
        tf.equal(ydim, 1), lambda: 1 - ybar, lambda: tf.one_hot(indices, ydim, on_value=1.0, off_value=0.0)
    )
    ...
    xadv, _ = tf.while_loop(_cond, _body, (xadv, 0), back_prop=False, name="fast_gradient_target")
    return xadv
